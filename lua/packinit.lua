...
require('packer').startup({
    function(use)
        ...
        use { 'neoclide/coc.nvim', config = "require('plugins/coc').setup()", branch = 'release' }

        -- wilder 弹出式命令行
        use { 'gelguy/wilder.nvim', event = 'CmdlineEnter', config = 'require("plugins/wilder").setup()' }

        -- github copilot
        -- require('plugins/copilot').config()
        -- use { 'github/copilot.vim', config = "require('plugins/copilot').setup()", event = 'InsertEnter' }

        -- 浮动终端
        require('plugins/vim-floaterm').config()
        use { 'voldikss/vim-floaterm', config = "require('plugins/vim-floaterm').setup()" }

        -- fzf
        require('plugins/fzf').config()
        use { 'junegunn/fzf', event = "CmdLineEnter" }
        use { 'junegunn/fzf.vim', config = "require('plugins/fzf').setup()", run = 'cd ~/.fzf && ./install --all', after = "fzf" }

        -- tree-sitter
        require('plugins/tree-sitter').config()
        use { 'nvim-treesitter/nvim-treesitter', config = "require('plugins/tree-sitter').setup()", run = ':TSUpdate', event = 'BufRead' }
        use { 'nvim-treesitter/playground', after = 'nvim-treesitter' }

        -- markdown预览插件 导航生成插件
        require('plugins/markdown').config()
        use { 'mzlogin/vim-markdown-toc', ft = 'markdown' }
        use { 'iamcco/markdown-preview.nvim', config = "require('plugins/markdown').setup()", run = 'cd app && yarn install', cmd = 'MarkdownPreview', ft = 'markdown' }

        -- 文件管理器
        require('plugins/nvim-tree').config()
        use { 'kyazdani42/nvim-web-devicons' }
        use { 'kyazdani42/nvim-tree.lua', config = "require('plugins/nvim-tree').setup()", cmd = { 'NvimTreeToggle', 'NvimTreeFindFileToggle' } }

        -- 状态栏 & 标题栏
        require('plugins/nvim-lines').config()
        use { 'yaocccc/nvim-lines.lua', config = "require('plugins/nvim-lines').setup()" }

        -- 部分个人自写插件
        require('plugins/yaocccc').config()                                               -- yaocccc/* 共用一个config
